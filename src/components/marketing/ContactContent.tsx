                ...
                <div className="grid lg:grid-cols-2 gap-12 max-w-5xl mx-auto">
                    ...
                    >
                        ...
                        {CHANNELS.map(({ icon: Icon, label, value, color }, i) => (
                            <motion.div
                                ...
                                className="flex items-start gap-4 p-5 bg-navy-card border border-navy-border rounded-2xl hover:border-white/10 transition-colors"
                            >
                    ...
                    >
                        {sent ? (
                            ...
                            <form onSubmit={handleSubmit} className="bg-navy-card border border-navy-border rounded-2xl p-7 space-y-5 shadow-2xl">
                                <h2 className="font-display font-bold text-white text-xl">Send a message</h2>
                                <div className="grid sm:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <Label className="text-white/70 text-sm">Full Name *</Label>
                                        <Input placeholder="John Doe" value={name} onChange={(e: any) => setName(e.target.value)}
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Input type="email" placeholder="you@example.com" value={email} onChange={(e: any) => setEmail(e.target.value)}
                                            className="h-11 bg-navy/60 border-navy-border text-white placeholder:text-white/20 rounded-xl focus:border-brand-blue" />
                                    </div>
                                </div>
                                <div className="space-y-1.5">
                                    <Label className="text-white/70 text-sm">Subject</Label>
                                    <Input placeholder="What's this about?" value={subject} onChange={(e: any) => setSubject(e.target.value)}
                                        className="h-11 bg-navy/60 border-navy-border text-white placeholder:text-white/20 rounded-xl focus:border-brand-blue" />
                                </div>
                                <div className="space-y-1.5">
                                ...
                                <Button type="submit" disabled={loading}
                                    className="w-full h-12 bg-brand-blue hover:bg-blue-600 text-white font-semibold rounded-xl shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all hover:-translate-y-0.5">
